...
- Plugin／Skill本体は`vendor/shared-agent-plugins`（git submodule、確定SHA）にあり、ここでは編集しない。共有資産の変更は`target-registry.json`の`shared-agent-plugins` targetを通じ、`shared-agent-plugins`側のrelease policyとR0–R3 Risk gateに従う。独立reviewとvalidationが完了し、targetとautomation policyの両方が`direct`を明示的に許可している場合は、PRを作らず`main`へ直接releaseしてよい。
...
4. 共有targetは`ensure_target_repo.py . shared-agent-plugins --apply`で`.ai-intelligence/ai-development/workspaces/`へcloneし、`prepare_target.py`でclean baseを確認してから使う。releaseは`release_git.py`でtargetの設定済みstrategyに従う。このAuthorityでは独立reviewとvalidationの完了後に`direct`で`main`へreleaseする。force-pushやrepository protectionの回避はしない。
